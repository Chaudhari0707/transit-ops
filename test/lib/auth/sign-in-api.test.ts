...
/** Better Auth + DB round-trips regularly take 2–5s; default 5s bun timeout flakes under load. */
...
describeWithDatabase("sign-in API integration", () => {
  ...
  afterAll(async () => {
    await deleteTestAuthUser(sql, fixtureEmails.dispatcher);
    await deleteTestAuthUser(sql, fixtureEmails.inactive);
    await deleteTestAuthUser(sql, fixtureEmails.deleted);
    await deleteTestAuthUser(sql, fixtureEmails.rateLimit);
    await sql.end();
  }, 60_000);

  test(
    "accepts valid admin credentials with matching role",
    async () => {
  ...
  test(
    "rejects valid password when selected role does not match",
    async () => {
      const response = await postSignInEmail({
        email: adminEmail,
  ...
  test(
    "rejects missing role header after credential verification",
    async () => {
      const response = await postSignInEmail({
        email: fixtureEmails.dispatcher,
  ...
  test(
    "rejects invalid role header value",
    async () => {
      const response = await postSignInEmail({
        email: fixtureEmails.dispatcher,
  ...
  test(
    "accepts non-admin users when role matches",
    async () => {
      const response = await postSignInEmail({
        email: fixtureEmails.dispatcher,
...
describeWithDatabase.serial("sign-in API rate limiting", () => {
  ...
  test(
    "locks out repeated failed password attempts",
    async () => {
