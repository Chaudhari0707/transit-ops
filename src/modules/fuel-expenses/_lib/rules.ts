import type { AppUserRole } from "@/modules/auth/_types/session";

/** Mockup screen 6 + RBAC: FA writes; FM/FA can view costs. */
export function assertFuelExpenseReadRole(role: AppUserRole): void {
 ...
 */
export function assertFuelExpenseWriteRole(role: AppUserRole): void {
  if (role !== "financial_analyst" && role !== "fleet_manager") {
    throw new Error("Forbidden");
  }
}
