---
name: boneyard
description: Loading states for TransitOps. HTML data tables use structure-preserving TableLoadingRows (not boneyard div overlays). boneyard-js CLI/registry still apply for non-table block capture. Never use @/components/ui/skeleton for ops tables. Applies when adding loading states, placeholders, skeletons, or boneyard build.
license: MIT
---

# Loading states (TransitOps)

## Hard rule: HTML data tables

**Do not wrap `<table>` / data grids in `boneyard-js/react` `<Skeleton>`.**

`<Skeleton>` is a **`<div>`**. Wrapping only the body requires two tables → **column width collapse** (truncated headers) and **layout shift**. Production learning from Drivers page.

### Required table pattern

1. One `<table>` for loading + loaded.
2. Always render: filters, forms, **card title/description**, **`<thead>`**.
3. While loading, only `<tbody>` uses `TableLoadingRows` from `@/lib/boneyard/table-row-shimmer`.
4. Bar colors/shimmer from `@/lib/boneyard/runtime-style` (`BONEYARD_RUNTIME`) — boneyard palette, not random grays / not CSS `animate-pulse` alone.
5. Never unmount chrome during load.

```tsx
import { TableLoadingRows } from "@/lib/boneyard/table-row-shimmer";

// inside Card + Table with real TableHeader:
<TableBody>
  {loading ? (
    <TableLoadingRows columnCount={9} rowCount={6} />
  ) : rows.length === 0 ? (
    <EmptyRow />
  ) : (
    rows.map(...)
...
Full agent policy: `src/app/AGENTS.md` → **Loading skeletons (tables & lists)**.

---

# boneyard-js (CLI / non-table blocks)

Pixel-perfect skeleton overlays extracted from real DOM. Still useful for **non-table** block UIs when chrome stays outside and one continuous block loads.

## How it works

1. Wrap your component with `<Skeleton>` and give it a `name`
2. Optionally add a `fixture` prop with mock data for the build step
3. Run `bunx --bun boneyard-js build` — it crawls your app, snapshots every named Skeleton, and writes `.bones.json` files + a `registry.js`
...
function BlogPage() {
  ...
  return (
    <Skeleton name="blog-card" loading={isLoading} fixture={<BlogCard data={MOCK_DATA} />}>
...
Or pass a URL explicitly: `bunx --bun boneyard-js build http://localhost:5173`
...
```tsx
<nav data-no-skeleton>{/* No bone will be generated for this element */}</nav>
```
...
### bunx --bun boneyard-js build options

```
bunx --bun boneyard-js build [url] [options]
...
Generate bones: `bunx --bun boneyard-js build --native --out ./bones`, then open your app on device. The Skeleton component auto-scans in dev mode — walks the React fiber tree, measures each view via UIManager, and sends bone data to the CLI. In production, scan code is completely inactive.
...
Native Preact integration — uses `preact/hooks` directly, no `preact/compat` needed. Same API as React. Same CLI: `bunx --bun boneyard-js build`. Works with the Vite plugin.
...
Uses Svelte 5 snippets for `fallback` and `fixture`. Same CLI: `bunx --bun boneyard-js build`.
...
Custom breakpoints: `bunx --bun boneyard-js build --breakpoints 390,820,1440`
