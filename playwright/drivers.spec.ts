...
test.describe("Drivers UI (authenticated)", () => {
  ...
  test("UI create driver appears in table", async ({ page }) => {
    ...
    // Category select may already have the first seed category applied on load.
    const categoryTrigger = page.getByRole("combobox", { name: /license category/i });
    await expect(categoryTrigger).toBeVisible();
    const categoryText = (await categoryTrigger.innerText()).trim();
    if (!categoryText || /select category/i.test(categoryText)) {
