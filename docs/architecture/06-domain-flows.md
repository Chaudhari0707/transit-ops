...
3. **Dispatcher** creates trip: source location `Gandhinagar Depot`, destination `Ahmedabad Hub` (from `locations` dropdown), vehicle, driver, cargo 450 kg, planned distance. Status `draft`. Source ≠ destination enforced.
